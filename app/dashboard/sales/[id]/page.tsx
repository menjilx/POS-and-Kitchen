    ...
    <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6">
      ...
      {receiptSettings && (
        <div className="block fixed -left-625 top-0 z-9999 bg-white p-0 m-0 w-full h-full overflow-hidden print:left-0 print:inset-0">
          <PrintableReceipt settings={receiptSettings} data={buildReceiptData()} />
        </div>
      )}
      <div className="print:hidden space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-bold truncate">Order {sale.order_number}</h1>
                <Badge
              ...
              <p className="text-sm text-muted-foreground">
                {new Date(sale.sale_time).toLocaleString()} &middot; {cashierLabel}
              </p>
            </div>
          </div>
          <div className="sm:ml-auto flex items-center gap-2 flex-wrap">
            ...
            {canDelete ? (
              <Button variant="outline" className="border-red-300 text-red-600 hover:bg-red-50" onClick={() => setDeleteDialogOpen(true)} disabled={deletingSale}>
                Delete
              </Button>
            ) : null}
          </div>
                ...
                <table className="w-full">
                  ...
                  <tfoot>
                    <tr className="border-t">
                      ...
                      <td className="text-right py-3">{formatCurrency(subtotal, currency)}</td>
                    </tr>
            ...
            <Card>
              ...
              <CardContent>
                {sale.kds_orders && sale.kds_orders.length > 0 ? (
                    ...
                    {sale.kds_orders.map((kds, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                          ...
                          >
                            <SelectTrigger className={`h-8 w-[130px] text-xs capitalize font-medium ${
                              kds.status === 'served' ? 'border-green-300 text-green-700 bg-green-50' :
                              kds.status === 'ready' ? 'border-blue-300 text-blue-700 bg-blue-50' :
                              kds.status === 'preparing' ? 'border-yellow-300 text-yellow-700 bg-yellow-50' :
                              kds.status === 'cancelled' ? 'border-red-300 text-red-700 bg-red-50' :
                              ''
                            }`}>
                              <SelectValue />
                            </SelectTrigger>
                  ...
                  <p className="text-muted-foreground text-sm">No kitchen orders found.</p>
                )}
              </CardContent>
            </Card>
                    ...
                    {saleHistory.map((entry) => {
                      ...
                      return (
                        <div key={entry.id} className="flex items-start justify-between gap-4 border-b pb-3 last:border-0 last:pb-0">
                          <div>
                            <p className="font-medium text-sm">{resolveHistoryAction(entry.action)}</p>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="border-t pt-3 space-y-3">
                  <div className="space-y-1.5">
                    ...
                    >
                      <SelectTrigger className={`h-9 w-full capitalize font-medium text-sm ${
                        sale.payment_status === 'paid' ? 'border-green-300 text-green-700 bg-green-50' :
                        sale.payment_status === 'voided' ? 'border-red-300 text-red-700 bg-red-50' :
                        sale.payment_status === 'refunded' ? 'border-orange-300 text-orange-700 bg-orange-50' :
                        sale.payment_status === 'pending' ? 'border-yellow-300 text-yellow-700 bg-yellow-50' :
                        ''
                      }`}>
                        <SelectValue />
                      </SelectTrigger>
                ...
                {/* Payment Details */}
                {(parsedPaymentData?.ref || parsedPaymentData?.notes || sale.payment_notes ||
                  typeof parsedPaymentData?.receivedAmount === 'number' || sale.notes) && (
                  <div className="border-t pt-3 space-y-2 text-sm">
                    {typeof parsedPaymentData?.receivedAmount === 'number' && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Received</span>
                        <span className="font-medium">{formatCurrency(parsedPaymentData.receivedAmount, currency)}</span>
                      </div>
                    )}
                    {typeof parsedPaymentData?.changeAmount === 'number' && parsedPaymentData.changeAmount > 0 && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Change</span>
                        <span className="font-medium">{formatCurrency(parsedPaymentData.changeAmount, currency)}</span>
                      </div>
                    )}
                    {parsedPaymentData?.ref && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Ref</span>
                        <span className="font-medium font-mono">{parsedPaymentData.ref}</span>
                      </div>
                    )}
                    {(sale.payment_notes || parsedPaymentData?.notes) && (
                      <div>
                        <p className="text-muted-foreground mb-0.5">Note</p>
                        <p className="text-sm bg-muted/50 rounded px-2 py-1.5">{sale.payment_notes || parsedPaymentData?.notes}</p>
                      </div>
                    )}
                    {sale.notes && (
                      <div>
                        <p className="text-muted-foreground mb-0.5">Order Notes</p>
                        <p className="text-sm bg-muted/50 rounded px-2 py-1.5">{sale.notes}</p>
                      </div>
                    )}
                  </div>
                )}
